...
anigenActual.prototype.eventContextMenu = function(event) {
	if(event.target.isChildOf(popup.container, true) || event.target.isChildOf(overlay.container.parentNode, true) || event.target instanceof HTMLInputElement || event.target instanceof HTMLTextAreaElement) { return; }
	event.preventDefault ? event.preventDefault() : event.returnValue = false;
